...
class ModifiedResNet(nn.Module):
    ...
    def __init__(
        self, layers, output_dim, heads, input_resolution=224, width=64, dropout=0
    ):
        super().__init__()
        ...
        self.attnpool = AttentionPool2d(
            input_resolution // 32, embed_dim, heads, output_dim, dropout=dropout
        )
...
class AttentionPool_Modified(nn.Module):
    def __init__(
        self,
        spacial_dim: int,
        embed_dim: int,
    ...
    def forward(self, x):
        ...
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC
        x = x + self.positional_embedding[:, None, :].to(x.dtype)  # (HW+1)NC
        x1, _ = F.multi_head_attention_forward(
            query=x[:1],
            key=x,
            value=x,
            embed_dim_to_check=x.shape[-1],
...
class ModifiedResNet_woPool(nn.Module):
    ...
    def __init__(
        ...
        self.attnpool = AttentionPool_Modified(
            input_resolution // 32, self.embed_dim, heads, output_dim, dropout=dropout
        )
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            width, layers, heads, dropout=dropout, drop_path=drop_path
        )
...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = ModifiedResNet_woPool(
